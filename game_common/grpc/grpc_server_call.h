...
public:
	...
	void Init(std::string& server_address) {
		...
		thread_ = std::thread(&grpc_server_call::ThreadFunc,this);
	}
	...
	void AddService(std::function<void(ServiceType*, ServerContext*, RequestType*,
		...
		new AsyncServerUnaryCall<RequestType,ResponseType>(request_unary, process_rpc);
	}
...
class AsyncServerUnaryCall final : public IAsyncServerCall {
 public:
  ...
  void DoInvoke() {
	  ...
	  response_writer_.Finish(reply_, status, grpc_server_call::Tag(this));
  }
